...
import config
from miniture_wati_templates import marketing_campaign_wati, marketing_campaign_wati_plain
import logging
...
def marketing_campaign_cron():
    ...
    # timeval = #row['Time']
    # status = #row['Sent?']
    template_name = 'funfeb_sale'#row['WATI template']
    # skus = str(row['SKU'])
    sku_payload = None
    ...
    #                                           number_lists=[])
    status_response = marketing_campaign_wati_plain(wati=wati, template=template_name)
